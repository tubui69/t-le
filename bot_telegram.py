...
async def handle_msg(update, ctx):
    ...
    results = {'xingtu': [], 'duolingo': [], 'wink': [], 'wink_account': [],
               'meitu': [], 'meitu_account': []}
    types = [detect_type(u) for u in urls]
    agent_urls = [u for u, t in zip(urls, types) if t in ('wink', 'meitu')]
    other_urls = [u for u, t in zip(urls, types) if t not in ('wink', 'meitu')]
    agent_kind = detect_type(agent_urls[0]) if agent_urls else None
    # Che do winkmk: gop tat ca link dai ly cung loai thanh 1 order
    if is_winkmk and agent_urls:
        first_url = agent_urls[0]
        extra = agent_urls[1:] if len(agent_urls) > 1 else None
    ...
    type_labels = {
        ...
        'meitu_account': '\U0001f510 Meitu SDT+MK+OTP:'
    }
